package com.mk.friendcircle.exception;
...
/**
 * <p>使用举例:</p>
 * <p>构造方法: {@link #BusinessException(int, String)}, {@link #BusinessException(ExceptionEnum)}和{@link #BusinessException(ExceptionEnum, String)}
 * <pre>
 * throw new BusinessException(code, message); // 自定义响应码和响应描述信息
 * throw new BusinessException(ExceptionEnum.SYSTEM_ERROR); // 抛出某类型的异常
 * throw new BusinessException(ExceptionEnum.BUSINESS_ERROR, "rpc接口异常");  // 抛出某类型的异常, 并加上具体描述, 最终描述信息将变成: "业务异常:rpc接口异常"
 * throw new BusinessException(ExceptionEnum.INVALID_PARAMETER, "uid不能为空"); // 抛出某类型的异常, 并加上具体描述, 最终描述信息将变成: "参数错误:uid不能为空"
 * </pre></p>
 *
 * @author shisong
 * @since 2018-11-20
